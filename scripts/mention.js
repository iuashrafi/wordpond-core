    ...
    $.fn.extend({
        mention: function(options) {
            this.opts = {
                ...
                sensitive: true,
                emptyQuery: false,
                // queryBy: ['display_name', 'user_nicename'],
                queryBy: ['display_name', 'user_nicename'],
                typeaheadOpts: {}
            };
            ...
            var settings = $.extend({}, this.opts, options),
                        ...
                        if (itemProps[settings.queryBy[i]]) {
                            ...
                            if ( !! usernames) {
                                for (j = 0; j < usernames.length; j++) {
                                    var user_nicename = (usernames[j].substring(1)).toLowerCase(),
                                        re = new RegExp(settings.delimiter + item, "g"),
                                        used = ((this.query.toLowerCase()).match(re));

                                    if (item.indexOf(user_nicename) != -1 && used === null) {
                                        return true;
                                    }
                                }
                            }
                    ...
                    if (items.length && settings.sensitive) {
                        ...
                        if (currentUser.length == 1) {
                            for (i = 0; i < len; i++) {
                                ...
                                if ((currentRes.user_nicename[0] == currentUser)) {
                                    priorities.highest.push(currentRes);
                                }
                                else if ((currentRes.user_nicename[0].toLowerCase() == currentUser.toLowerCase())) {
                                    priorities.high.push(currentRes);
                                }
                                else if (currentRes.user_nicename.indexOf(currentUser) != -1) {
                ...
                _render = function(items) {
                    ...
                    items = $(items).map(function(i, item) {
                        ...
                        i = $(that.options.item).attr('data-value', item.user_nicename);
                        ...
                        if (item.display_name) {
                            _linkHtml.append('<b class="mention_name">' + item.display_name + '</b>');
                        }
                        if (item.user_nicename) {
                            _linkHtml.append('<span class="mention_username"> ' + settings.delimiter + item.user_nicename + '</span>');
                        }
